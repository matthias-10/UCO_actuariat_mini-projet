...
% Simulation pas a pas pour l'affichage
for i = 2:(n+1)
...
P = exp(-r * T ) * mean(XK);
sd = exp(-r * T) * std(XK);
i_inf = P - 1.96 * sd / sqrt(nb_trajectoires);
i_sup = P + 1.96 * sd / sqrt(nb_trajectoires);
